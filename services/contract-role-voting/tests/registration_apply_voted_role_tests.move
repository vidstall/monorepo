...
/// contract.
///
/// Package split: `apply_voted_role` no longer takes `&mut RoleVoteBox` directly (see its
/// own doc comment in registration.move). Every call site here now derives `new_role` via
/// `role_voting::consume_voted_assignment(&mut box, &cap)` first, simulating the daemon's
/// real 2-command PTB as two sequential Move calls within the same test transaction. This
/// test module lives in package B's (`dvconf_role_voting`) test tree, not package A's --
/// `dvconf_role_voting` already depends on `dvconf_contracts` one-directionally for its
/// production build, so calling `dvconf::registration::apply_voted_role` (a plain `public
/// fun`) from here is a normal same-direction dependency, unlike the reverse (a
/// `[dev-dependencies]` entry on package A pointing back at B), which the Sui Move
/// toolchain rejects outright as a cyclic dependency even though it's test-only.
///
/// Coverage:
...
#[test_only]
module dvconf_role_voting::registration_apply_voted_role_tests {
    ...
    fun apply_01_relay_to_validator_cleans_old_and_transitions() {
        ...
        ts::next_tx(&mut scenario, MINER);
        {
            ...
            let mut stake = ts::take_shared<StakePosition>(&scenario);

            let new_role = role_voting::consume_voted_assignment(&mut box, &cap);
            registration::apply_voted_role(
                &registry, &mut store, new_role,
