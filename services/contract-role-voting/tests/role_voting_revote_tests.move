...
#[test_only]
module dvconf_role_voting::role_voting_revote_tests {
    ...
    use dvconf_role_voting::role_voting_revote;
    ...
    fun test_3_2_9_full_e2e_relay_to_validator() {
        ...
        {
            let cap = caps::new_cp_cap_for_testing(id_from_addr(CP1_ID), ts::ctx(&mut scenario));
        ...
        ts::next_tx(&mut scenario, h::admin());
        {
            ...
            role_voting_revote::mark_revote_eligible_idle(
        ...
        ts::next_tx(&mut scenario, MINER);
        let stake = ts::take_shared<StakePosition>(&scenario);
        ...
        // StakePosition is now a shared object -- no need to stash it back to a
        // specific address between transactions; just return it to the shared pool
        // and any later transaction (including apply_voted_role's) can take it again.
        ts::next_tx(&mut scenario, CP1_OP);
        ...
        ts::next_tx(&mut scenario, MINER);
        {
            ...
            // Package split: apply_voted_role no longer takes &mut RoleVoteBox directly --
            // the daemon's real 2-command PTB is simulated here as two sequential calls
            // within the same test transaction (see role_voting.move's module doc).
            let new_role = role_voting::consume_voted_assignment(&mut vote_box, &cap);
