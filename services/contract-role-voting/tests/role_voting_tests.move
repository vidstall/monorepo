...
#[test_only]
module dvconf_role_voting::role_voting_tests {
    ...
    fun setup_voting(): ts::Scenario {
        ...
        {
            let cap = caps::new_cp_cap_for_testing(id_from_addr(CP2_ID), ts::ctx(&mut scenario));
            transfer::public_transfer(cap, CP2_OP);
        };
        ts::next_tx(&mut scenario, CP3_OP);
        {
            let cap = caps::new_cp_cap_for_testing(id_from_addr(CP3_ID), ts::ctx(&mut scenario));
    ...
    fun test_unregistered_cp_rejected() {
        ...
        {
            let cap = caps::new_cp_cap_for_testing(fake_cp_id, ts::ctx(&mut scenario));
