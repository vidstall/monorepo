...
class ServicePort(TypedDict):
    ...
    # (TopologyInstance.desired_state is "running" if ANY colocated worker
    # wants to run) -- Ansible needs this to correctly pause/resume one
    # container without disturbing others sharing the host.
    desired_state: str
    # 1-based; distinguishes multiple colocated workers of the SAME
    # service on one host (vidctl.py's count-prefix --service syntax, e.g.
    # `5cp-daemon`). index=1 is the pre-existing single-worker identity.
    index: int
...
class TopologyInstance(TypedDict, total=False):
    # The VM/host this worker runs on (topology.toml's `[[workers]]` field,
    # cli/infra.py's `host`). `name` (below) is a DISTINCT field kept for the
    # unrelated `[[frontends]]`/`objects` rows this same loose TypedDict is
    # also reused for (frontend/bucket site identity) -- worker rows use
    # `host`, frontend rows use `name`; a single dict never needs both.
    host: str
    name: str
    service: str
    provider: str
    ...
    # rows (cli/infra.py's new_worker always sets it, default 1).
    worker_index: int
    # Runtime-only: populated by program.py's group-by-host merge for
