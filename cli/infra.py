...
def control_many_hosts(
    ...
    failed_stage = "pulumi"
    code = pulumi_up(env_name, parallel=20)
    ...
    if code == 0:
        ...
        if code == 0:
            for item in prepared:
                persist_vm_resolution(topology, env_name, item["host"], item["service"], item["provider"])
        if code == 0:
            failed_stage = "configure"
            container_state = "restarted" if action == "restart" else "started"
            ...
            code = configure(host_limit=host_limit, container_state=container_state, extra_vars=extra_vars)
    ...
    if code == 0:
        for item in prepared:
            ...
            if wallet_entry is None:
                continue
            address = host_address(item["host"])
            wallet_address = str(wallet_entry.get("address", ""))
            try:
                balance_mist = wallet.current_balance_mist(wallet_address)
            except (subprocess.CalledProcessError, json.JSONDecodeError):
            ...
            print(f"Wallet:  {wallet_address[:8]}...")
            print(f"Balance: {balance_mist / 1_000_000_000:.4f} SUI")
            print(f"Registry: {registry_status(item['host'], item['worker_key'], address)}")
