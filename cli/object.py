...
from . import infra
from .context import command_env, contract_env_path, read_env_file, run

OBJECT_TYPES = {"frontend": infra.DOCKER_SERVICES["frontend"]}
...
def publish(name: str, object_type: str, provider: str) -> int:
    ...
    missing_keys = infra.missing_contract_keys(contract_path)
    if missing_keys:
        message = (
    ...
    obj["contract_env"] = infra.relative_contract_env(env_name)
    set_object_storage_defaults(obj, env_name)
    infra.write_topology(topology)

    code = build_static_artifacts(object_type, env_name)
    if code == 0:
...
def set_object_storage_defaults(obj: dict[str, Any], env_name: str) -> None:
    ...
    obj.setdefault("artifact_dir", f"services/{obj.get('object', 'frontend')}/out")
...
def build_static_artifacts(object_type: str, env_name: str) -> int:
    contract_path = contract_env_path(env_name)
